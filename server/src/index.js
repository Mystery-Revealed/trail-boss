...
app.get('/healthz', (_req, res) =>
  res.json({ ok: true, service: 'trail-boss', liveSessions: manager.registry.sessions.size }));
...
server.listen(config.port, () => {
  console.log(`[trail-boss] listening on :${config.port} — sessions live in memory only`);
});
